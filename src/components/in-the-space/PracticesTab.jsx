...
export default function PracticesTab({
  ...
  // Define items BEFORE startVoiceGuidance to avoid "Cannot access uninitialized variable" error
  const items = useMemo(() => {
  ...
  const startVoiceGuidance = useCallback(
    async (voiceAudioId) => {
      ...
      if (option?.isAI && selectedCardIndex !== null && practiceDuration !== null && items) {
        try {
